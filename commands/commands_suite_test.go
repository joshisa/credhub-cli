...
const CERTIFICATE_CREDENTIAL_WITH_NAMED_CA_REQUEST_JSON = `{"type":"certificate","name":"%s","value":{"ca":"", "ca_name":"%s","certificate":"%s","private_key":"%s"}}`
const CERTIFICATE_CREDENTIAL_WITH_NAMED_CA_REQUEST_JSON_FOR_IMPORT = `{"type":"certificate","name":"%s","value":{"ca_name":"%s","certificate":"%s","private_key":"%s"}}`
...
const SET_RESPONSE_JSON = `{"type":"%s","id":"` + UUID + `","name":"%s","version_created_at":"` + TIMESTAMP + `","value":%s}`
